...
export default {
    ...
    methods:{
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            console.log(1)
            this.axios.post('/url/api/maker/updatecakind',qs.stringify({
                cachetkindid:this.cachetkindid,
    ...
    created(){
        console.log('参数',this.$route.params.cachetKindid)
         this.cachetkindid= this.$route.params.cachetKindid
        this.axios.post('/url/api/maker/selcakindby',{
            cachetkindid:this.cachetkindid
        }).then(res=>{
           console.log("更新数据",res.data)
            if (res.data.status == 'success') {
                this.ruleForm.cachetKind=res.data.data[0].cakindname
                this.ruleForm.explain=res.data.data[0].cachetExplain
            }
        }).catch(err=>{
            console.log(err)
        })
